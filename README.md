...
首次启动发现 `warp-v2ray/config/config.json` 不存在时，容器会生成一份默认配置
（随机 UUID）**写回该目录**，并在日志里打印 UUID 与 `vmess://` 分享链接：

```
[entrypoint] 未找到 /config/config.json，按首次部署处理，自动生成默认配置 ...
[entrypoint] 已生成默认配置并写入 /config/config.json
[entrypoint] ────────────────────────────────────────────────
[entrypoint] VMess UUID : 3f2a....
[entrypoint] VMess 分享链接（导入 v2rayN / v2rayNG）：
[entrypoint]   vmess://eyJ2IjoiMiIsInBzIjoid2FycC12MnJheSIs...
[entrypoint] SOCKS5 : <本机IP>:9001  （无认证，支持 UDP）
[entrypoint] HTTP   : <本机IP>:9002  （无认证）
[entrypoint] ────────────────────────────────────────────────
```

配置落盘后 UUID 跨重启稳定。设 `SERVER_ADDR=<你的IP>` 可让分享链接直接可用，
否则链接里的地址是占位符，导入后需手动改。

> `/config` **不能挂成 `:ro`**，否则写不回去，配置只存在于容器内、每次重启 UUID 都会变。
> 真发生时启动日志会明确警告。

### 从现有 v2ray 迁移

```bash
# 放入现有配置，inbounds 会被原样保留，客户端零改动
mkdir -p warp-v2ray/config warp-v2ray/data
cp /path/to/your/v2ray/config.json warp-v2ray/config/
...
`CONFIG_MODE=patch`（默认）下，容器读取挂载的 `warp-v2ray/config/config.json`（不存在则先自动生成一份），只做三件事：
...
| 变量 | 默认 | 说明 |
|---|---|---|
| `CONFIG_MODE` | `patch` | `patch`：有配置就用、没有就自动生成一份；`generate`：无条件重新生成（**会覆盖已有配置**） |
| `VMESS_UUID` | 自动生成 | 自动生成时指定 UUID；留空则随机生成一个 |
| `SERVER_ADDR` | — | 本机 IP/域名，仅用于让日志里的 `vmess://` 分享链接直接可用 |
...
在 Docker Hub 建一个 Access Token（**别用账号密码**），权限 Read & Write，然后：

```bash
gh secret set DOCKERHUB_USERNAME --repo qhkly/docker_warp-v2ray --body "land007"
gh secret set DOCKERHUB_TOKEN    --repo qhkly/docker_warp-v2ray   # 交互式粘贴 token
```

没配 Secrets 之前，workflow 会在登录那步失败——这是预期行为，不影响仓库其它内容。

workflow 最后有一步**硬校验 manifest 里两个架构都在**。推送成功不代表 manifest 完整，少一个架构照样算"成功"，要等用户在那个平台上 `docker pull` 才会暴露，所以显式检查。
